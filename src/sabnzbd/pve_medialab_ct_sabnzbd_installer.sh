#!/usr/bin/env bash
# ----------------------------------------------------------------------------------
# Filename:     pve_medialab_ct_sabnzbd_installer.sh
# Description:  This script is for creating a Proxmox SABnzbd CT
# ----------------------------------------------------------------------------------
...
# SABnzbd SW installer
pct exec $CTID -- bash -c "export REPO_PKG_NAME=$REPO_PKG_NAME APP_USERNAME=$APP_USERNAME APP_GRPNAME=$APP_GRPNAME && /tmp/$GIT_REPO/src/$REPO_PKG_NAME/sabnzbd_sw.sh"

#---- Pre-configure SABnzbd

# Configure SABnzbd
pct exec $CTID -- bash -c "export REPO_PKG_NAME=$REPO_PKG_NAME APP_USERNAME=$APP_USERNAME APP_GRPNAME=$APP_GRPNAME GIT_REPO=$GIT_REPO && /tmp/$GIT_REPO/src/$REPO_PKG_NAME/config/sabnzbd_config.sh"


# Check Install CT SW status (active or abort script)
pct_check_systemctl "sabnzbdplus.service"
...
#---- Set display text
# Get port
port=$(pct exec $CTID -- crudini --get /etc/default/sabnzbdplus "" PORT)
...
# Web access URL
display_msg1=( "http://$(pct exec $CTID -- hostname).$(pct exec $CTID -- hostname -d):$port/sabnzbd/" )
display_msg1+=( "http://$(pct exec $CTID -- hostname -I | sed -r 's/\s+//g'):$port/sabnzbd/ ($ip_type)" )

# Backup preset description
display_msg2=( "--  General ${REPO_PKG_NAME^} configuration and settings" \
"--  Includes Ahuacate and Trash Guides customization" \
"--  Doesn't include 3rd party account details (i.e usenet servers)" \
"--  Sets backup destination: /mnt/backup/${REPO_PKG_NAME,,} (preset to use NAS)" )
# Check CT domain
if [ ! $(hostname -d) == 'local' ]
then 
display_msg2+=( "--  Note: The default preset file uses the '.local' domain." \
...
Your ${REPO_PKG_NAME^} is pre-configured except for your Usenet server account details. Your preset includes:

$(printf '%s\n' "${display_msg2[@]}" | indent2)

If you are performing reinstallation of ${REPO_PKG_NAME^} you can use the application's built-in restoration function. Open the ${REPO_PKG_NAME^} WebGUI and navigate to the 'Backup & Restore' function. From there, select the option to 'Restore Backup'. In the backup section that appears, click 'Browse' and navigate to your NAS folder '.../proxmox/backup/${REPO_PKG_NAME^}' where backup files are stored.
