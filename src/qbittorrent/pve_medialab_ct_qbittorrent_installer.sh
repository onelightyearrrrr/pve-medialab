#!/usr/bin/env bash
# ----------------------------------------------------------------------------------
# Filename:     pve_medialab_ct_qbittorrent_installer.sh
# Description:  This script is for creating a Proxmox qBittorrent CT
# ----------------------------------------------------------------------------------
...
pct push $CTID $SRC_DIR/qbittorrent/qbittorrent_sw.sh /tmp/qbittorrent_sw.sh -perms 755
pct exec $CTID -- bash -c "export REPO_PKG_NAME=$REPO_PKG_NAME APP_USERNAME=$APP_USERNAME APP_GRPNAME=$APP_GRPNAME && /tmp/qbittorrent_sw.sh"
...
pct_check_systemctl "qbittorrent-nox.service"
...
display_msg1+=( "Web login username : 'admin' password: 'adminadmin'" )
